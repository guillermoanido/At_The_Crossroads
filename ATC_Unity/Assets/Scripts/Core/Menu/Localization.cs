...
public static class Localization
{
    ...
    private static readonly Dictionary<string, string[]> ui = new Dictionary<string, string[]>
    {
        ...
        ["builder.filter_str"]   = new[] { "STR", "FUE" },
        ["builder.filter_dex"]   = new[] { "DEX", "DES" },
        ["builder.filter_int"]   = new[] { "INT", "INT" },
        ["builder.filter_wis"]   = new[] { "WIS", "SAB" },
        ["builder.hint"]         = new[] { "Click a card to add it  ·  right-click to remove  ·  click a deck row to remove",
                                           "Clic para añadir  ·  clic derecho para quitar  ·  clic en una fila del mazo para quitar" },
        ...
        // In-match log — impersonal: it reports what happened, not who did it. {0}/{1} are filled
        // in at runtime, so keep the placeholders in both forms.
        ["log.played"]           = new[] { "Played {0}", "{0} jugada" },
        ["log.damage"]           = new[] { "{0} damage dealt", "{0} daño hecho" },
        ["log.damage_blocked"]   = new[] { "{0} damage dealt, {1} blocked", "{0} daño hecho, {1} bloqueado" },
